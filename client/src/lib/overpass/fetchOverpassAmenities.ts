...
/**
 * POST to Overpass mirrors until one succeeds. Handles slow servers and polite headers.
 */
export async function fetchOverpassAmenitiesJson(): Promise<OverpassResult> {
  const query = AMENITY_OVERPASS_QUERY;
  let lastFail: { status: number; snippet: string } = {
    status: 0,
    snippet: "",
  };
